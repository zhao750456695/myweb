...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        if register_form.is_valid():
            ...
            user_message.message = "欢迎注册杰信在线网"
            user_message.save()
...
class MyCourseView(LoginRequiredMixin, View):
    ...
    '''
    def get(self, request):
        allcourses = UserCourse.objects.filter(user=request.user)
        return render(request, 'usercenter-mycourse.html', {'allcourses': allcourses})
...
class FavOrgView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        org_list = []
        fav_orgs = UserFavorite.objects.filter(user=request.user, fav_type=2) #fav_type=2 筛选收藏的机构
        # UserFavorite里没有course_org 只有fav_id
        for fav_org in fav_orgs:
            org_id =fav_org.fav_id
            org = CourseOrg.objects.get(id=org_id) # 筛一个是get 多个是filter
            org_list.append(org)
...
class FavTeacherView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        for fav_teacher in fav_teachers:
            ...
            teacher = Teacher.objects.get(id=teacher_id) # 筛一个是get 多个是filter
...
class FavCourseView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        for fav_course in fav_courses:
            ...
            course = Course.objects.get(id=course_id) # 筛一个是get 多个是filter
